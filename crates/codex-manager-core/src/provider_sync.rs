...
pub fn run_provider_sync() -> ProviderSyncResult {
    run_provider_sync_for_home(&paths::codex_home_dir())
}

pub fn run_provider_sync_for_home(home: &Path) -> ProviderSyncResult {
    if !home.exists() {
        return result(
    ...
    sync_result.unwrap_or_else(|error| {
        result(
            ProviderSyncStatus::Skipped,
            format!("Provider sync skipped: {error}"),
            &target_provider,
            None,
            0,
            0,
...
fn result(
    ...
    ProviderSyncResult {
        ...
        sync_message: sync_message.into(),
    }
}
...
fn create_backup(
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(
            &json!({"managedBy": "Codex Manager provider sync", "targetProvider": target_provider}),
        )?,
    )?;
    Ok(backup_dir)
}
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("Codex Manager provider sync") {
            managed.push(path);
        }
    }
...
#[cfg(test)]
mod tests {
    use super::{ProviderSyncStatus, run_provider_sync_for_home};
    use rusqlite::Connection;
    use serde_json::json;
    use std::fs;
    use std::path::Path;
    ...
    fn create_state_db(path: &Path) {
        ...
        .unwrap();
        db.execute(
            "INSERT INTO threads VALUES ('thread-1', 'openai', 0, 0, 'C:/old')",
            [],
        )
        .unwrap();
    }
    ...
    #[test]
    fn provider_sync_updates_rollout_and_sqlite_visibility() {
        ...
        let result = run_provider_sync_for_home(&home);

        assert_eq!(result.sync_status, ProviderSyncStatus::Synced);
        assert_eq!(result.target_provider, "CodexManager");
        ...
        .unwrap();
        assert_eq!(first["payload"]["model_provider"], "CodexManager");
        ...
        assert_eq!(
            row,
            ("CodexManager".to_string(), 1, "C:/workspace".to_string())
        );
